...
AI organization requires a configured provider, API key, model, and endpoint. API keys are stored locally with `@AppStorage` in this version. A future release should move secrets to Keychain.
...
AI 整理功能需要服务商、API key、模型和 endpoint 都配置正确。当前版本使用 `@AppStorage` 本地保存 API key，后续版本建议迁移到 Keychain。
