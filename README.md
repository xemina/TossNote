...
Drop in text, links, images, PDFs, Office documents, or mixed local files. TossNote extracts what it can, sends the combined context to your chosen AI provider, and returns organized, editable Markdown with summaries, structure, and tags. It is built for people who want to save information first and sort it later, with one-click save to Obsidian, a local folder, or Joplin.
...
- Use Quick Save to organize and save in one step when review is not needed.
...
- Switch between English/Simplified Chinese in Settings, and toggle light/dark mode from the main toolbar.
...
AI organization requires a configured provider, API key, model, and endpoint. API keys are stored in the app's local preferences using lightweight encoding to avoid macOS Keychain permission prompts.
...
- `VERSION`
- `Sources/TossNote/App/AppVersion.swift`
- `CHANGELOG.md`
...
- API keys and the Joplin Web Clipper token are stored locally in the app's preferences with lightweight encoding.
- This avoids macOS Keychain permission prompts in unsigned or locally built app versions.
...
- 使用一键保存，在不需要预览时直接整理并保存。
- 保存前可以直接编辑 Markdown。
- 支持保存到 Obsidian、本地文件夹和 Joplin Web Clipper。
- 可以在设置中切换英文/简体中文，并在主工具栏切换白天/黑夜模式。
...
AI 整理功能需要服务商、API key、模型和 endpoint 都配置正确。API key 会保存在应用本地偏好中，并做轻量编码，避免未签名或本地构建版本触发 macOS Keychain 权限弹窗。
...
- API key 和 Joplin Web Clipper token 会保存到应用本地偏好中，并做轻量编码。
- 这样可以避免未签名或本地构建版本触发 macOS Keychain 权限弹窗。
